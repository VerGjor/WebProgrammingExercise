...
import {getStudentsFromApi, createStudent} from "../../repository/studentsApi";
...
class StudentList extends React.Component{
    ...
    constructor (props) {
      ...
      componentWillReceiveProps(nextProps){
        if(this.props.studentList !== nextProps.studentList)
            this.setState({
                listOfStudents: nextProps.studentList,
                list: nextProps.studentList.map((item, index) => {
                    return (
    ...
    render(){
        ...
        <h2 className="mx-auto text-center">Student information</h2>
        <h3 className="mx-auto text-center">Web Programming
        </h3>
        <div className="form-group mx-auto text-center">
                    <button className="btn bg-dark text-light" onClick={this.showAddStudentButton.bind(this)}>Add Student</button>
        </div>
        <hr/>
        <table className="table float-left col-3 mt-2 text-center table-bordered table-hover">
         ...
         <div className="col-5 float-right mt-2">
             {this.state.isHidden &&
                <EditStudentDetails studentID = {this.state.listOfStudents[this.state.id].index}
                    changeInfo = {this.changeInfo} />}

            {this.state.addStudentButton && <AddStudent student='' onNewStudent = {this.onNewStudent}/>}
         </div>
